...
import {changeSortCriterion, clearSelectedOptions, getPizzas} from "../../../redux/action";
...
function SortPopup({sortCriteria, dispatch, currentSortCriterion}) {
    ...
    useEffect(() => {
        setCurrentSortCriterionObj(sortCriteria.find(criterion => criterion.id === currentSortCriterion));
    }, [sortCriteria])
    ...
    useEffect(() => {
        dispatch(getPizzas(currentSortCriterionObj));
    }, [currentSortCriterionObj]);
