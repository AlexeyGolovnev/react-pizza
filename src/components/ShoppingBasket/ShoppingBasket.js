...
import {
    addPizzaToBasket,
    calculateTotalData, clearBasket,
    deleteOnePizzaFromBasket, deletePizzaFromBasket
} from "../../redux/action";
...
function ShoppingBasket() {
    ...
    let selectedPizzasJsx = useMemo(() => selectedPizzas.map(selectedPizza => {

        let pizza = pizzas.find(pizza => pizza.id === selectedPizza.pizzaId);
        let dough = doughTypes.find(dough => dough.id === selectedPizza.doughId);
        ...
        return (
            <ShoppingBasketItem
                ...
                onAddPizza={() => dispatch(addPizzaToBasket(selectedPizza.pizzaId, selectedPizza.doughId, selectedPizza.sizeId, selectedPizza.price, selectedPizza.pizzasCount))}
                onDeleteOnePizza={() => dispatch(deleteOnePizzaFromBasket(selectedPizza.pizzaId, selectedPizza.doughId, selectedPizza.sizeId))}
                onDeletePizza={() => dispatch(deletePizzaFromBasket(selectedPizza.pizzaId, selectedPizza.doughId, selectedPizza.sizeId))}
            />
        )
    }), [selectedPizzas]);
                    ...
                    : <div className="basket__inner">
                        ...
                        <div className="basket__content">
                            ...
                            <div className='basket__content-total'>
                                <span className='basket__content-total-count'>Всего пицц: <b>{totalCount} шт</b>.</span>
                                <span
                                    className='basket__content-total-price'>Сумма заказа: <b>{totalPrice} руб</b>.</span>
                            </div>
